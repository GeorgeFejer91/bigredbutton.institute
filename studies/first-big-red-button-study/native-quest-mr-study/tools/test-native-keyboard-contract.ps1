...
$demographicsKeypressStressScriptText = if (Test-Path $demographicsKeypressStressScriptPath) {
    Get-Content -Raw -LiteralPath $demographicsKeypressStressScriptPath
} else {
    ''
}
...
$sourceChecks = [ordered]@{
    ...
    guidedNameToAgeFlow = $activityText.Contains('requiredTextField') -and $activityText.Contains('requestDemographicsTextInputFocus("age", "name_submit_next")') -and $activityText.Contains('isRequired = requiredTextField == "name"') -and $activityText.Contains('isRequired = requiredTextField == "age"') -and -not $activityText.Contains('name_valid_auto_advance')
    demographicsHardwareKeyFallback = $activityText.Contains('override fun dispatchKeyEvent(event: KeyEvent)') -and $activityText.Contains('handleDemographicsHardwareKeyEvent(event)') -and $activityText.Contains('appendDemographicsNameCharacter') -and $activityText.Contains('backspaceDemographicsName') -and $activityText.Contains('appendDemographicsAgeDigit') -and $activityText.Contains('activity_key_event') -and $activityText.Contains('KEYCODE_NUMPAD_ENTER')
    ...
    questKeyboardKeypressStressUsesHardwareFallback = (Test-Path $demographicsKeypressStressScriptPath) -and $demographicsKeypressStressScriptText.Contains("Send-TextToken 'George'") -and $demographicsKeypressStressScriptText.Contains("Send-KeyCode 62 'SPACE'") -and $demographicsKeypressStressScriptText.Contains("Send-TextToken 'Fejer'") -and $demographicsKeypressStressScriptText.Contains('source=activity_key_event') -and $demographicsKeypressStressScriptText.Contains("Send-KeyCode 66 'ENTER_NEXT'") -and $demographicsKeypressStressScriptText.Contains("Send-KeyCode 66 'ENTER_DONE'")

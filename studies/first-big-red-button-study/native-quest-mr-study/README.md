...
2. One-time pre-condition prompt: asks about prior Big Red Button experience, plays the selected feedback clip, then plays `pre_start_instructions.mp3` before `Start experiment` appears.
...
- Name and Age use visible Android `EditText` fields inside the Quest panel and request the Quest virtual keyboard as text/Next and number/Done fields. Participant ID is generated under the hood; gender is `male`, `female`, `other`, or `prefer_not_to_say`; handedness is `left`, `right`, or `ambidextrous`.
...
- Fast Quest directional/data validation includes native Name text-keyboard evidence, Age numeric-keyboard evidence, panel-exit keyboard hide evidence, enter-submit replay evidence, `exportMirrorMatched=true`, exact ECG audio-window proof, feedback counterbalance/sham blink runtime flash proof, press `elapsed_ns` export proof, and the updated pictographic replay expectation: condition 2 right/right/up/up exports closeness `40` and presence `60`.

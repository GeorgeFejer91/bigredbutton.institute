...
- Current redness-note: the Button Experience task includes `How red did the button feel?`. Condition 1 starts with a VAS (`slightly red` to `very red`) and converts once to a seven-box Likert scale; condition 2 starts as Likert and converts once to VAS. Export both final values plus `rednessScaleOrder`. The supplied changeover clips are active as `first_questionnaire_change` and `second_questionnaire_change_excuse`; participant runs play the full clips with timed visual choreography, while qkv fast replay logs `validationShortcut=true` and uses a short navigation cue so it does not wait through those clips.

...
5. `../README.md`
6. `../docs/validation-ladder.md`
7. `../docs/completion-audit.md`
8. `../docs/visual-validation-notes.md`
9. `../docs/physical-validation-operator-guide.md`
...
- Current questionnaire/input note: demographics is a compact no-scroll panel; name and age use only the native Quest/system keyboard, not an embedded or custom keyboard inside the questionnaire. The retired custom loose-keyboard implementation has been removed from the activity, and `tools/test-native-keyboard-contract.ps1` now fails if it comes back. The central questionnaire stays in gaze line; focusing Name requests text input, focusing Age requests numeric input, and the app forces IME retarget/restart so the movable system keyboard switches modes reliably while remaining participant movable/closable. Age still sanitizes typed input to digits and max three characters.
- Current pictographic note: the horizontal control is a distance-from-self visual axis. Slider thumb and button marker move together; exports keep `feltCloseness0To100` as higher-is-closer and store visible distance separately as `selfButtonDistanceUnits`. Endpoint labels are `very close`/`very distant` and `small presence`/`large presence`.
- Current redness-note: the Button Experience task includes `How red did the button feel?`. Condition 1 starts with a VAS (`slightly red` to `very red`) and converts once to a seven-box Likert scale; condition 2 starts as Likert and converts once to VAS. Export both final values plus `rednessScaleOrder`. The apology/supervisor audio clip is still pending; the app currently logs/plays `redness_scale_conversion_pending_apology` as a placeholder cue.
- Current button input note: controller-contact remains the final physical proof source, but hand-tracked collider selects are accepted and exported as separate `hand_contact` provenance for usability.
- Current heartbeat visual note: heartbeat/R-peak flashes use warm model-shaped surface-emission overlays, not a flat halo.

...
For Quest demographics text entry, the durable rule is one visible editable owner per value, with source checks proving that owner. This project moved back to the reliable 2D APK pattern on 2026-06-12: visible `AndroidView(EditText)` fields embedded in the BRB warm-paper shell for both Name and Age. Name uses `InputType.TYPE_CLASS_TEXT | TYPE_TEXT_FLAG_CAP_WORDS`, `IME_ACTION_NEXT`, and max 80. Age uses `TYPE_CLASS_NUMBER`, `IME_ACTION_DONE`, digits-only sanitizer, and max 3. Name Next and stable valid Name request Age focus; retargeting calls `InputMethodManager.restartInput(editText)` and logs `platformControl=EditText`, `focusedView=EditText`, `inputOwner=androidViewEditText`, and `restartInput=true`. Validate this with `tools/test-native-keyboard-contract.ps1`, `tools/run-quest-demographics-keyboard-entry-validation.ps1`, and qkv comparisons for Name text keyboard plus Age numeric keyboard evidence. The native keyboard contract should fail if `SpatialTextField` demographics ownership, hidden bridge ownership, Age trigger-dial ownership, custom loose keyboard ownership, or any mixed-owner path returns. If manual headset typing still fails after this, the next intentional fallback is a visible app-owned single keyboard with rectangular letters and a digit pad writing directly to highlighted Name/Age state; do not add another hidden IME bridge.
...
For heartbeat-driven button visuals, prefer model-shaped warm emission over a flat overlay. In this project, the current accepted workaround for the lack of loaded-GLB material mutation is 32 opaque GLB material variants plus small native surface lights, not transparent Compose or mesh halo overlays. ECG/RR feedback is a glow-only contract: do not call the GLB `pressed` animation on the glow variants, and keep visible geometry motion reserved for accepted button presses. The runtime uses `BUTTON_PRESS_MOTION_GLOW_SUPPRESSION_MS` to let press motion win briefly over any concurrent heartbeat glow. Keep `BRB_HEARTBEAT_FLASH` as timing evidence and inspect a real headset screenshot for gross framing.

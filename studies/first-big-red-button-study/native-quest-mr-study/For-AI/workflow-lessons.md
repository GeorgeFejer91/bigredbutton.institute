...
For heartbeat-driven button visuals, prefer model-shaped warm emission over a flat overlay. In this project, the current accepted workaround for the lack of loaded-GLB material mutation is 32 opaque GLB material variants plus small native surface lights, not transparent Compose or mesh halo overlays. ECG/RR feedback is a glow-only contract: do not call the GLB `pressed` animation on the glow variants, and keep visible geometry motion reserved for accepted button presses. Heartbeat glow may continue during a press; the separation is about animation ownership, not suppressing glow. Keep `BRB_HEARTBEAT_FLASH` as timing evidence and inspect a real headset screenshot for gross framing.

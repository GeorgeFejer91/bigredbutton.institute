...
When a task deliberately converts between response formats, store both final answers and the order instead of overwriting one with the other. The redness task uses `rednessVas0To100`, `rednessLikert1To7`, `rednessLikertDescriptor`, and `rednessScaleOrder`, and qkv validation compares both conversion directions. Now that the real conversion clips are supplied, hash-lock both the source/provenance copy and the packaged Android raw resource, log the transcript-derived choreography timings, and disable saving during participant-facing changeover audio. Keep fast validation immediate only when it logs `validationShortcut=true`; do not make routine qkv wait through nonessential questionnaire-change clips.

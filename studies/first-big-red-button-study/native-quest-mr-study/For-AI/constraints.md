...
- Demographics text entry currently uses the native Quest/system keyboard through visible `AndroidView(EditText)` controls embedded in the questionnaire panel. Name and Age must each have one real visible `EditText` owner; do not mix this with UISet `SpatialTextField` ownership or a hidden bridge for the same values. Name must use text/capitalized-words input, `IME_ACTION_NEXT`, and max 80 characters. Age must use numeric input, `IME_ACTION_DONE`, digits-only sanitizer, and max 3 characters. Keep required-field highlighting plus guided Name-to-Age handoff so participants are never left guessing which control is active. Keep `BRB_SYSTEM_KEYBOARD_FIELD_CONTRACT`, `BRB_DEMOGRAPHICS_EDITTEXT_FOCUS_REQUEST`, `BRB_DEMOGRAPHICS_EDITTEXT_FOCUS`, `BRB_SOFT_KEYBOARD_REQUEST`, `BRB_SOFT_KEYBOARD_RETARGET`, `BRB_DEMOGRAPHICS_AGE_FILTER`, and `BRB_DEMOGRAPHICS_TEXT_VALUE` markers as lifecycle/sanitizer evidence. `tools/test-native-keyboard-contract.ps1` and `tools/validate-study.ps1` should fail if `useLooseKeyboard`, `requestLooseKeyboard`, `LooseKeyboard`, `BRB_LOOSE_KEYBOARD`, hidden bridge ownership, `SpatialTextField` demographics ownership, Age trigger-dial ownership, or any other mixed-owner path reappears. If manual Quest IME still fails, a visible app-owned single keyboard with rectangular letters and a digit pad may be introduced only as an intentional study-design change with updated validation and screenshots.
...
- Heartbeat/R-peak flashes should read as warm surface emission from the button body, not as a flat 2D halo or transparent canopy. In the current Meta Spatial SDK path, prefer GLB material variants with brighter/emissive cap materials; use true loaded-material mutation only if a future native runtime exposes it safely and it is headset-validated.
...
- The one-time prior Big Red Button experience prompt must keep the 3D model/contact collider hidden, hide Yes/No choices until the question clip finishes, hide `Start experiment` until the selected feedback clip and `pre_start_instructions.mp3` both finish, and occur only before condition 1.
